...
export default function App() {
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    if (targetUrl.startsWith('https://github.com/')) {
        try {
            // Strip out [Scanning Multiple] prefix if the user clicked analyze twice
            if (targetUrl.includes('[Scanning Multiple]:')) {
            ...
            if (pathParts.length === 2) {
                ...
                try {
                  ...
                  if (treeRes.ok) {
                      ...
                      let topFiles = riskyFiles.slice(0, 5);
                      if (topFiles.length === 0) topFiles = ['README.md'];
                      
                      targetUrl = topFiles.map((f: string) => `https://raw.githubusercontent.com/${owner}/${repo}/main/${f}`).join(',');
                      setUrl(`[Scanning Multiple]: ${topFiles.join(', ')}`);
                  } else {
      ...
      <div className="max-w-2xl w-full relative z-10">
        ...
        >
          ...
          <div className="flex gap-4 sm:gap-6 text-sm font-medium text-slate-500 justify-center flex-wrap">
             <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-indigo-500/70" /> Top-5 Risk Analysis</span>
