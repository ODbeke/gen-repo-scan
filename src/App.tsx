...
interface ScanResult {
  status: "SECURE" | "NOT SECURE" | "PENDING" | "NOT_FOUND" | "FETCH_FAILED";
...
export default function App() {
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    if (targetUrl.startsWith('https://github.com/')) {
        try {
            if (targetUrl.includes('[Scanning Repo]:')) {
            ...
            if (pathParts.length === 2) {
                ...
                try {
                  ...
                  if (treeRes.ok) {
                      ...
                      // Select the top 2 highest risk files
                      let topFiles = riskyFiles.slice(0, 2);
                      if (topFiles.length === 0) topFiles = ['README.md'];
                      
                      const scanUrls: string[] = topFiles.map(
                        (file) => `https://raw.githubusercontent.com/${owner}/${repo}/main/${file}`
                      );
                      
                      let commitInfo = '';
                      try {
                        setLoadingStep('Locating latest commit...');
                        const commitRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/commits?per_page=1`);
                        if (commitRes.ok) {
                          const commitData = await commitRes.json();
                          if (commitData && commitData.length > 0) {
                            const sha = commitData[0].sha;
                            // Add git diff URL for commit history scanning
                            const diffUrl = `https://github.com/${owner}/${repo}/commit/${sha}.diff`;
                            scanUrls.push(diffUrl);
                            commitInfo = `, commit: ${sha.slice(0, 7)}`;
                          }
                        }
                      } catch (err) {
                      ...
                      targetUrl = scanUrls.join(',');
                      setUrl(`[Scanning Repo]: ${topFiles.join(', ')}${commitInfo}`);
                  } else {
    ...
    try {
      ...
      setLoadingStep('Retrieving Audit Logs...');
      ...
      <div className="max-w-2xl w-full relative z-10">
        ...
        >
          ...
          <p className="text-slate-400 text-lg max-w-xl leading-relaxed mb-8">
            Paste any GitHub repository link. Our autonomous AI validators will fetch your codebase, scan files & commit diffs for secrets, and reach on-chain consensus.
          </p>
          <div className="flex gap-4 sm:gap-6 text-sm font-medium text-slate-500 justify-center flex-wrap">
             <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-indigo-500/70" /> Commit History & Diff Scans</span>
             <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-indigo-500/70" /> Multi-File Consensus</span>
        ...
        >
          <form onSubmit={handleAnalyze} className="space-y-6">
            <div className="relative">
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">GitHub Repository or Raw URL</label>
                ...
                <div className="text-center">
                  ...
                  <p className="text-slate-400 font-medium">
                    {result.status === 'SECURE' 
                      ...
                      : result.status === 'FETCH_FAILED'
                      ? result.error || 'Failed to fetch the target files. Please check if the repository or file is public.'
                      : 'Critical security risks identified in the source code.'}
                  </p>
                </div>
              </div>

              {/* Scanned Files Audit Logs */}
              {result.scanned_files && result.scanned_files.length > 0 && (
                <div className="mt-6 mb-6 p-4 bg-slate-950/40 rounded-xl border border-slate-800/40 text-left">
                  <span className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Scanned Target Files & Diffs</span>
                  <div className="flex flex-col gap-2 font-mono text-xs text-slate-400">
                    {result.scanned_files.map((file, idx) => {
                      let displayName = file;
                      if (file.includes('/commit/')) {
                        const parts = file.split('/commit/');
                        displayName = `Commit Diff: ${parts[1].slice(0, 7)}.diff`;
                      } else if (file.includes('/main/')) {
                        const parts = file.split('/main/');
                        displayName = `File: ${parts[1]}`;
                      }
                      return (
                        <div key={idx} className="flex items-center gap-2">
                          <CheckCircle2 className="w-4 h-4 text-indigo-500" />
                          <span className="truncate block max-w-full">{displayName}</span>
                        </div>
                      );
                    })}
                  </div>
        ...
        {/* Consensus Receipt Inspector Modal */}
        <AnimatePresence>
          {showReceiptModal && rawReceipt && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            >
              <motion.div
                ...
                className="bg-slate-900 border border-slate-800 w-full max-w-2xl rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]"
              >
                {/* Modal Header */}
                <div className="p-6 border-b border-slate-800 bg-slate-950/50 flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <Shield className="w-5 h-5 text-indigo-400" />
                    <h3 className="font-bold text-lg text-slate-200">GenLayer Consensus Proof</h3>
                  </div>
                  <button
                    onClick={() => setShowReceiptModal(false)}
                    className="text-slate-500 hover:text-slate-300 transition-colors text-sm font-semibold"
                  >
                    Close
                  </button>
                </div>
                ...
                <div className="p-6 overflow-y-auto space-y-6 text-left">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="p-4 bg-slate-950/30 border border-slate-850 rounded-xl">
                      <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Leader Node Result</span>
                      <span className="font-mono text-sm text-indigo-400 truncate block">
                        {rawReceipt?.consensus_data?.leader_receipt?.[0]?.result?.payload?.readable || 'N/A'}
                      </span>
                    </div>
                    <div className="p-4 bg-slate-950/30 border border-slate-850 rounded-xl">
                      <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Consensus Status</span>
                      <span className="font-mono text-sm text-green-400 font-bold block">
                        {rawReceipt?.statusName || rawReceipt?.status_name || 'ACCEPTED'}
                      </span>
                    </div>
                  </div>

                  {/* Decoded/Structure Inspector */}
                  <div>
                    <span className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Consensus Details</span>
                    <div className="space-y-3 font-mono text-xs bg-slate-950/50 p-4 rounded-xl border border-slate-850 text-slate-400">
                      <div className="flex justify-between border-b border-slate-800/30 pb-2">
                        <span className="text-slate-500">Transaction Hash:</span>
                        <span className="text-slate-300 break-all select-all ml-4">{rawReceipt?.hash || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between border-b border-slate-800/30 pb-2">
                        <span className="text-slate-500">Gas Used:</span>
                        <span className="text-slate-300">
                          {rawReceipt?.gasUsed?.toString() || rawReceipt?.gas_used?.toString() || '0'}
                        </span>
                      </div>
                      <div className="flex justify-between border-b border-slate-800/30 pb-2">
                        <span className="text-slate-500">Leader Address:</span>
                        <span className="text-slate-300 break-all select-all ml-4">
                          {rawReceipt?.consensus_data?.leader_receipt?.[0]?.validator_address || 'N/A'}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-500">Total Validator Votes:</span>
                        <span className="text-slate-300 font-bold">
                          {rawReceipt?.consensus_data?.leader_receipt?.length || '1'}
                        </span>
                      </div>
                    </div>
                  </div>

                  {/* Base64 Raw Receipt Payload */}
                  <div>
                    <span className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Raw Base64 Proof</span>
                    <div className="relative group">
                      <textarea
                        readOnly
                        value={getBase64Receipt()}
                        className="w-full h-32 bg-slate-950/70 border border-slate-800 rounded-xl p-3 font-mono text-[10px] text-slate-500 focus:outline-none resize-none select-all"
                      />
                      <button
                        onClick={() => navigator.clipboard.writeText(getBase64Receipt())}
                        type="button"
                        className="absolute bottom-3 right-3 bg-indigo-600/80 hover:bg-indigo-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        Copy Base64
                      </button>
                    </div>
